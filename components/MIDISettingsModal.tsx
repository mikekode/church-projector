...
export default function MIDISettingsModal({ isOpen, onClose, midi }: MIDISettingsModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-2xl w-full max-w-2xl mx-4 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 flex-shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-zinc-500 dark:text-zinc-400">Configure Hardware & Controllers</p>
                        </div>
                    </div>
                ...
                {/* Tabs */}
                <div className="flex border-b border-zinc-200 dark:border-white/5 bg-zinc-50 dark:bg-zinc-950/30 flex-shrink-0">
                    <button
                        onClick={() => setActiveTab('midi')}
                        className={`flex-1 py-3 text-sm font-bold uppercase tracking-wider transition-colors border-b-2 ${activeTab === 'midi' ? 'border-purple-500 text-purple-600 dark:text-purple-400 bg-purple-500/5' : 'border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-white/5'}`}
                    >
                        MIDI Control
                    </button>
                    <button
                        onClick={() => setActiveTab('hardware')}
                        className={`flex-1 py-3 text-sm font-bold uppercase tracking-wider transition-colors border-b-2 ${activeTab === 'hardware' ? 'border-indigo-500 text-indigo-600 dark:text-indigo-400 bg-indigo-500/5' : 'border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-white/5'}`}
                    >
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    {activeTab === 'midi' && (
                        <div className="animate-in fade-in slide-in-from-left-4 duration-200">
                            {/* Toggle Switch */}
                            <div className="flex items-center justify-between mb-8 p-4 bg-zinc-100 dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-white/5">
                                <div className="flex items-center gap-3">
                                    <Radio className={`w-5 h-5 ${isEnabled ? 'text-green-500 dark:text-green-400 animate-pulse' : 'text-zinc-400 dark:text-zinc-500'}`} />
                                    <span className="font-medium text-zinc-700 dark:text-white">Enable MIDI Input</span>
                                </div>
                                <label className="relative inline-flex items-center cursor-pointer">
                                    ...
                                    />
                                    <div className="w-11 h-6 bg-zinc-300 dark:bg-zinc-700 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-purple-800 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-purple-600"></div>
                                </label>
                            </div>

                            {/* Device Status */}
                            <div className="mb-8">
                                <h3 className="text-sm font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-3">Connected Devices</h3>
                                {inputs.length === 0 ? (
                            ...
                            <div className="mb-8 p-4 bg-zinc-100 dark:bg-zinc-950 rounded-lg border border-zinc-200 dark:border-zinc-800 font-mono text-sm">
                                <div className="text-zinc-500 mb-1 text-xs uppercase">Last Received Signal</div>
                                {lastMsg ? (
                                    <div className="flex gap-4 text-purple-600 dark:text-purple-400">
                            ...
                            {/* Mappings */}
                            <div>
                                <h3 className="text-sm font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-4">Command Mappings</h3>
                                <div className="grid gap-3">
                                    {[
                                        ...
                                        return (
                                            <div key={cmd.id} className="flex items-center justify-between p-3 bg-zinc-100 dark:bg-zinc-800/50 rounded-lg border border-zinc-200 dark:border-white/5">
                                                <span className="font-medium text-zinc-700 dark:text-zinc-300">{cmd.label}</span>
                                                <div className="flex items-center gap-3">
                                                    {map ? (
                                                        <span className="px-2 py-1 bg-zinc-200 dark:bg-zinc-700 rounded text-xs font-mono text-zinc-700 dark:text-zinc-300">
                    ...
                    {activeTab === 'hardware' && (
                        <div className="animate-in fade-in slide-in-from-right-4 duration-200 space-y-6">
                            {/* ATEM Section */}
                            <div className="bg-zinc-100 dark:bg-zinc-950/50 rounded-xl p-6 border border-zinc-200 dark:border-white/5">
                                ...
                                <div className="flex gap-2">
                                    <input
                                        className="flex-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded px-3 py-2 text-sm text-zinc-900 dark:text-white focus:border-indigo-500 outline-none font-mono"
                                        value={atemIp}
                ...
                <div className="p-4 border-t border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 text-right flex-shrink-0">
                    <button
                        onClick={onClose}
                        className="px-6 py-2 bg-zinc-900 dark:bg-white text-white dark:text-black font-bold rounded-lg hover:scale-105 transition-transform"
                    >
                        Done
                    </button>
                </div>
